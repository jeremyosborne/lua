...
listWordsReverse = function(s)
    ...
    local wordList = listWords(s)
    local reversed = {}
    for _,word in ipairs(wordList) do
        table.insert(reversed, 1, word)
    end
    return reversed
end
...
mapWords = function(s, op)
    local words = listWords(s)
    local mapped = {}
    for _,word in ipairs(words) do
        table.insert(mapped, op(word))
    end
    return mapped
end
...
--- Build a function that manages a variable named counter that is
-- not accessible to other things in this file, nor accessible to things
